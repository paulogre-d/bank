...
const ACCOUNTS: AccountOption[] = [
  { id: "checking", name: "Premium Checking", lastFour: "4589", balance: "$12,450.00" },
  { id: "savings", name: "High Yield Savings", lastFour: "9012", balance: "$45,200.50" },
  { id: "visa", name: "Visa Infinite", lastFour: "3456", balance: "-$1,250.00" },
];
...
export default function TransfersPage() {
  ...
  const internalAccounts = ACCOUNTS.filter((a) => a.id !== "visa");
  ...
  const getToLabel = () => {
    if (transferType === "internal") return `${toAcc.name} (${toAcc.lastFour})`;
    if (transferType === "person") return recipient || "—";
    if (transferType === "wire") return toAccountNumber ? `•••• ${toAccountNumber.slice(-4)}` : "—";
    return "—";
  };

  const handleReviewTransfer = () => setShowReview(true);
  const handleBack = () => setShowReview(false);
  const handleConfirmTransfer = () => {
    setShowReview(false);
    setLastReferenceId(
      `#TRX-${Math.floor(1000 + Math.random() * 9000)}-${String(new Date().getFullYear()).slice(-2)}`
    );
          ...
          <div className="flex flex-col gap-6 py-8">
            ...
            <div className="flex gap-4">
              <button
                type="button"
                onClick={handleBack}
                className="h-12 flex-1 rounded-[14px] bg-transparent text-base font-normal text-[#45556C] transition hover:bg-[#F8FAFC]"
              >
                Back
              </button>
              <button
                ...
                className="h-12 flex-1 rounded-[14px] bg-[#0F172B] text-base font-bold text-white transition hover:bg-[#1E293B]"
              >
                Confirm Transfer
              </button>
            </div>
